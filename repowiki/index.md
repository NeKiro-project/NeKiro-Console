# NeKiro Console RepoWiki

This is the bilingual reading surface for the production React/Vite/TypeScript
Console. The Console repository owns the UI and browser behavior; Core does
not maintain a second writable Console source tree.

## Start here

- [Console documentation](console-docs/index.md) — API, architecture, operations, and development guides.
- [GitHub repository](https://github.com/NeKiro-project/NeKiro-Console) — source, issues, and releases.
- [Core RepoWiki](https://nekiro-project.github.io/NeKiro/) — platform contracts and architecture.

Detailed pages are rendered from the committed `.qoder/repowiki` export. The
English and Chinese site entry points share that canonical Console content.
