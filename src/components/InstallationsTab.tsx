...
import type {Agent, Installation, InstallationStatus, InstallIntent, PlatformErrorView, Workspace} from '../types';
...
interface InstallationsTabProps {
  ...
  initialSelection?: InstallIntent;
  onInstallAgent: (agent: Agent, release: AgentRelease, acceptedPermissions: string[]) => Promise<Installation>;
  ...
  onContinueToInvoke?: (installation: Installation) => void;
}
...
export default function InstallationsTab({
  ...
  const [recentInstallation, setRecentInstallation] = useState<Installation | null>(null);
  ...
  const handleSelectAgent = (selectedKey: string) => {
    ...
    setPreflightRelease(null);
    setLocalError(null);
    setRecentInstallation(null);
  };

  useEffect(() => {
    if (!initialSelection || appliedSelectionSequence.current === initialSelection.sequence) return;
    const agent = publishedAgents.find((item) => agentKey(item) === initialSelection.agentKey);
    if (!agent) return;
    appliedSelectionSequence.current = initialSelection.sequence;
    invalidatePreflight();
    setSelectedAgentKey(initialSelection.agentKey);
    setVersionConstraint(agent.version);
    setAcceptedPermissions([]);
    setReleaseId(initialSelection.releaseId);
    setLocalError(null);
    setRecentInstallation(null);
  }, [initialSelection, publishedAgents]);
  ...
  const handleInstall = async (event: React.FormEvent) => {
    ...
    setLocalError(null);
    try {
      const installation = await onInstallAgent(selectedAgent, preflightRelease, acceptedPermissions);
      setRecentInstallation(installation);
    } catch (installError) {
  ...
  return (
    <div className="h-full flex flex-col gap-5">
      ...
      {recentInstallation && isTrustedEnabledInstallation(recentInstallation) && onContinueToInvoke && (
        <div className="flex flex-wrap items-center justify-between gap-3 rounded-lg border border-green-400/30 bg-green-500/10 p-4 text-sm text-brand-on-surface-variant">
          <div><span className="font-semibold text-green-300">Agent installed.</span> The exact Release is enabled in {recentInstallation.workspaceId}.</div>
          <button type="button" onClick={() => onContinueToInvoke(recentInstallation)} className="flex items-center gap-2 rounded-lg bg-brand-primary px-4 py-2 text-xs font-semibold text-white"><PlayCircle size={14} /> Continue to Invoke</button>
        </div>
      )}
      ...
      <div className="glass-split-grid grid grid-cols-[minmax(360px,0.85fr)_minmax(520px,1.15fr)] gap-5 min-h-0 flex-1">
        <form onSubmit={handleInstall} className="bg-brand-low border border-brand-outline-variant rounded-xl p-4 h-fit">
          ...
          <div className="flex flex-col gap-1 text-xs text-brand-on-surface-variant mb-3">
            ...
            <div className="flex gap-2"><input id="trusted-release-id" value={releaseId} onChange={(event) => { invalidatePreflight(); setReleaseId(event.target.value); setLocalError(null); setRecentInstallation(null); }} disabled={!workspace || preflightLoading || busyLifecycle} placeholder="release-id" className="flex-1 bg-brand-lowest border border-brand-outline-variant rounded px-3 py-2 text-brand-on-surface outline-none disabled:opacity-50" /><button type="button" onClick={handlePreflight} disabled={!workspace || !selectedAgent || !releaseId || preflightLoading || busyLifecycle} className="px-3 py-2 rounded border border-brand-outline-variant bg-brand-container text-xs text-brand-on-surface-variant disabled:opacity-40">{preflightLoading ? 'Reading...' : 'Preflight'}</button></div>
          </div>
        ...
        <div className="bg-brand-low border border-brand-outline-variant rounded-xl overflow-hidden min-h-0 flex flex-col">
          ...
          <div className="overflow-y-auto divide-y divide-brand-outline-variant/40">
            {filteredInstallations.length === 0 ? (
              ...
              <div key={installation.installationId} data-installation-id={installation.installationId} className="p-4">
                ...
                <div className="mt-3 flex gap-2">
                  {isTrustedEnabledInstallation(installation) && onContinueToInvoke && <button disabled={busyLifecycle} onClick={() => onContinueToInvoke(installation)} className="flex items-center gap-1.5 rounded bg-brand-primary px-3 py-1.5 text-xs font-semibold text-white disabled:opacity-40"><PlayCircle size={12} />Invoke</button>}
